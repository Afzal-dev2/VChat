...
import com.malkinfo.vchat.adapter.MessagesAdapter
import com.malkinfo.vchat.databinding.ActivityChatBinding
import com.malkinfo.vchat.model.Message
...
class ChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Glide.with(this@ChatActivity).load(profile) //using glider library to load profile images
                .placeholder(R.drawable.avatar)
                .into(binding!!.profile01)
        binding!!.imageView2.setOnClickListener(View.OnClickListener { finish() })
        receiverUid = intent.getStringExtra("uid")
        senderUid = FirebaseAuth.getInstance().uid
        database!!.reference.child("presence").child(receiverUid!!) //getting data from database for user activity whether other user is online or offline
        ...
        binding!!.sendBtn.setOnClickListener(View.OnClickListener {
            ...
            val message = Message(name,messageTxt, senderUid, date.time)

            binding!!.messageBox.setText("")
            val randomKey = database!!.reference.push().key

            //hashmap to store certain data

            val lastMsgObj = HashMap<String, Any>()
            lastMsgObj["ruid"] = receiverUid.toString()
            lastMsgObj["profimg"] = profile.toString()
            lastMsgObj["uid"] = senderUid.toString()
            lastMsgObj["name"] = name.toString()
            lastMsgObj["lastMsg"] = message.message!!
            lastMsgObj["lastMsgTime"] = date.time
            database!!.reference.child("chats").child(senderRoom!!).updateChildren(lastMsgObj)

            database!!.reference.child("chats") //writing data to database
                    .child(senderRoom!!)
                    .child("messages")
                    .child(randomKey!!)
                    ...
                    reference.putFile(selectedImage!!).addOnCompleteListener { task ->
                        ...
                        if (task.isSuccessful) {
                            reference.downloadUrl.addOnSuccessListener { uri ->
                                ...
                                val date = Date()
                                val message = Message(name.toString(),messageTxt, senderUid, date.time)
                                message.message = "photo"
                                message.imageUrl = filePath //to send images
                                binding!!.messageBox.setText("")
                                val randomKey = database!!.reference.push().key
                                val lastMsgObj = HashMap<String, Any>()
                                lastMsgObj["ruid"] = receiverUid.toString()
                                lastMsgObj["profimg"] = profile
                                lastMsgObj["uid"] = senderUid.toString()
                                lastMsgObj["name"] = name
                                lastMsgObj["lastMsg"] = message.message!!
                                lastMsgObj["lastMsgTime"] = date.time
